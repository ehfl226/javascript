    ...
    let globalName = 'global name'; // 블럭 안 밖 모두에서 출력 가능(어플리케이션의 시작부터 끝날때까지 메모리에 탑제된 상태기 때문에 class나 함수 if나 for에 필요할때만 사용하는 것이 좋다.)
    {
      let name = 'yoon';
    ...
    // var (ES6d 이전에 사용 현재는 쓰지 않는게 좋다.) 
    // 대부분 프로그래밍 언어에서는 변수를 선언하고 나서 값을 할당해야하는데 var를 사용하면서 선언도 하기 전에 값을 할당하거나 console.log로 불러올 수 있었다.
    // undefined로 뜸 (변수는 정의되어있는데 값이 없다)

    // var hoisting (move declaration from bottom to top) - 어디에 선언했냐에 상관없이 항상 제일 위로 선언을 끌어올려 주는 것을 말한다.
    // has no block scope - 블럭을 이용해서 선언해도 어디서나 보인다!(규모있는 프로젝트를 하다보면 블럭안에 코드를 집어 넣어도 아무곳에서 불러와져서 에러가 생김)

    {
        console.log(age); // undefined - 선언은 했지만 값은 없음
        age = 4;
    ...
    // primitive, single item: number, string, boolean, null, undefiedn,symbol (값자체가 메모리에 저장)
    // object, box container (메모리에 레퍼런스가 담겨있고, 객체 안에 있는 값은 변경 가능하다. )
    ...
    const helloBob = `hi ${brendan}!`; // template literals (`string에 (${)쓰면 변수의 값이 붙여져서 나옴!}`)
    ...
    yoon.age = 31; // name, age 메모리에는 다른 값으로 변경 가능

    // 5. Dynamic typing: dynamically typed language (변수를 선언할때 어떤 type인지 선언하지않고 할당된 값에 따라서 type이 변경된다.)
    let text = 'hello';
